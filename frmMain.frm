...
Private Sub Form_Initialize()
   ...
   SetWindowPos Me.hWnd, HWND_.NOTOPMOST, 0, 0, 0, 0, False, False
...
Private Sub Form_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
   ...
   ElseIf Button = vbKeyMButton Then
      ...
      SetWindowPos frmSettings.hWnd, HWND_.NOTOPMOST, 0, 0, 0, 0, False, False
   End If
End Sub
...
Private Sub ProjectileClass_OnStrike(ByVal iProjectile As Long, ByVal jButton As Long, ByVal x As Single, ByVal y As Single, ByVal z As Single, ByVal r As Single)
   ...
   If foohittest = True Then
      ...
      If InStr(1, VBA.LCase(st), ".lnk") <> 0 Then
      ...
      If VBA.Trim(st) <> "" Then consoleClass.Display st
      Set fso = Nothing
      Set f = Nothing
      '        If (Len(st) > 0) Then
      '            Dim voice As Object
   ...
   Else
      ...
      If foo = True Then
         ...
         Dim s As String
         s = ThingPath(jButton)
         sParams = VBA.Chr(34) & s & VBA.Chr(34)
...
Private Sub EscapeDialogMenuWindow()
   ...
   If apiIsWindow(fwnd) <> 0 Then
      ...
      If wn.lpClass = "" Or wn.lpClass = "Windows.UI.Core.CoreWindow" Then
         ...
         If VBA.Trim(VBA.LCase(wn.lpText)) = "action center" Or VBA.Trim(VBA.LCase(wn.lpText)) = "windows shell experience host" Then
            winescape
            Exit Sub
         End If
      End If
   ...
   If lBufferLen >= 1 Then
      ...
      If RC <> 0 Then
         ...
         If RC <> 0 Then
            ...
            Lang_Charset_String = VBA.Hex(HexNumber)
            ...
            If VBA.Trim(GetFileInformation) = "" Then GetFileInformation = GetStringValue("\StringFileInfo\" & Lang_Charset_String & "\" & "ProductName", lVerPointer, lBufferLen, sBuffer)
            If VBA.Trim(GetFileInformation) = "" Then GetFileInformation = GetStringValue("\StringFileInfo\" & Lang_Charset_String & "\" & "OriginalFileName", lVerPointer, lBufferLen, sBuffer)
            If VBA.Trim(GetFileInformation) = "" Then GetFileInformation = GetStringValue("\StringFileInfo\" & Lang_Charset_String & "\" & "InternalName", lVerPointer, lBufferLen, sBuffer)
         End If
      End If
   End If
End Function
   ...
   If Picture1.TextWidth(StrText) > Picture1.ScaleWidth Then
      ...
      While Not x = 0
         If Picture1.TextWidth(left(StrText, Len(StrText) - x)) < Picture1.ScaleWidth Then
            StrText = left(StrText, Len(StrText) - x) & vbCrLf & VBA.right(StrText, x)
            x = 0
         Else
...
Private Function GetWinNameAPI(ByVal hWnd As Long, Optional ByVal gText As Boolean = True, Optional ByVal gClass As Boolean = True) As WINNAME
   ...
   If gText = True Then '''''''''''''''''''''''''If text is to be retrieved
      ...
      n.lpText = VBA.Space(260) ' n.lpText.PadLeft(tLength) '''''Pad with buffer
   ...
   If gClass = True Then ''''''''''''''''''''''''If class name is to be retrieved
      n.lpClass = VBA.Space(260) 'n.lpClass.PadLeft(260) '''''''Pad with buffer
...
Private Sub Isxinput(Optional ByVal hWnd As Long = 0)
   ...
   If IsNoRedirectionBitmap(hWnd) = True Then
      ...
      If wn.lpClass = "ApplicationFrameWindow" Then
         IsXINPUTSupported = False
         If VBA.LCase(wn.lpText) = "xbox" Then
            xenable = True
            modifierkeydown = False
         Else
            xenable = False
...
Private Function GetStringValue(ByRef searchString As String, ByVal lVerPointer As Long, ByVal lBufferLen As Long, ByRef sBuffer() As Byte) As String
   ...
   If RC <> 0 Then
      ...
      GetStringValue = VBA.Mid$(Buffer, 1, InStr(Buffer, VBA.Chr(0)) - 1)
   End If
End Function
...
Private Function GetEXEProcessID(ByVal sEXE As String) As Long
   ...
   If CheckVersion() = VER_PLATFORM_WIN32_WINDOWS Then
      ...
      Do While bCopied
         sName = left(tPE.szExeFile, InStr(tPE.szExeFile, VBA.Chr(0)) - 1)
         sName = VBA.Mid(sName, InStrRev(sName, "\") + 1)
         If InStr(sName, VBA.Chr(0)) Then
            sName = left(sName, InStr(sName, VBA.Chr(0)) - 1)
         End If
      ...
      For iIndex = 0 To lProcesses - 1
         ...
         If lprocess <> 0 Then
            If apiEnumProcessModules(lprocess, lModule, 4, 0&) Then
               ...
               If InStr(sName, "\") > 0 Then sName = VBA.Mid(sName, InStrRev(sName, "\") + 1)
               If InStr(sName, VBA.Chr(0)) Then sName = VBA.left(sName, InStr(sName, VBA.Chr(0)) - 1)
               If StrComp(sEXE, sName, vbTextCompare) = 0 Then
